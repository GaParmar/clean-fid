...
Below, We vary the format used for storing the generated images and plot the resulting changes in the FID of a StyleGAN2 model trained on the FFHQ dataset (left) and LSUN outdoor Church dataset (right). The images generated by the StyleGAN2 are resized 1024 to 299 using PIL-bicubic and stored using different formats before computing the FID score. Note that LSUN dataset images were collected with JPEG compression (quality 75), whereas FFHQ images were collected as PNG. The blue dashed line in both graphs shows the FID score when the generated images are quantized to 8-bit unsigned integers (PNG). Whereas the orange dashed line shows the FID score if the generated images are left unquantized as floating-point numbers. Interestingly, for LSUN dataset, the best FID score (3.48) is obtained when the generated images are compressed with JPEG quality 87.
